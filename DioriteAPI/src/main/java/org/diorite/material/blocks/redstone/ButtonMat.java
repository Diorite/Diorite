...
public abstract class ButtonMat extends BlockMaterialData implements PowerableMat, AttachableMat
{
    ...
    protected ButtonMat(final String enumName, final int id, final String minecraftId, final BlockFace face, final boolean powered)
    {
        super(enumName, id, minecraftId, face.name() + (powered ? "_POWERED" : ""), combine(face, powered));
        this.face = face;
        this.powered = powered;
    }

    protected ButtonMat(final String enumName, final int id, final String minecraftId, final int maxStack, final String typeName, final byte type, final BlockFace face, final boolean powered)
    {
