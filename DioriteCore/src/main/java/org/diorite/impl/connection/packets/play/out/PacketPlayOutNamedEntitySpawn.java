...
public class PacketPlayOutNamedEntitySpawn implements PacketPlayOut
{
    ...
    public void writePacket(final PacketDataSerializer data) throws IOException
    {
        ...
        final PacketDataSerializer data2 = new PacketDataSerializer(Unpooled.buffer(200));
        data2.writeVarInt(this.entityId);
        data2.writeUUID(this.uuid);
        data2.writeInt(this.x);
        data2.writeInt(this.y);
        data2.writeInt(this.z);
        data2.writeByte(this.yaw);
        data2.writeByte(this.pitch);
        data2.writeShort(this.currentItem);
        data2.writeEntityMetadata(this.metadata);
        final PacketPlayOutNamedEntitySpawn p =new PacketPlayOutNamedEntitySpawn();
        p.readPacket(data2);
